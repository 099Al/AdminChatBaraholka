...
- Через меню бота удаляет сообщения за последние 7 дней:
  - `Удалить некорректные сообщения` - удаляет сообщения без ключевых слов.
...
`BOT_TOKEN` берется у `@BotFather`. `MAIN_ADMIN_USER` - это Telegram user ID человека, которому разрешено управлять ботом. Его можно узнать командой `/get_id` в боте. `API_ID` и `API_HASH` берутся в кабинете Telegram: https://my.telegram.org -> `API development tools`.
...
- `Удалить некорректные сообщения` - удаляет за последние 7 дней сообщения без ключевых слов из `KEYWORDS`.
