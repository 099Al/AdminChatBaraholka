...
`BOT_TOKEN` берется у `@BotFather`. `API_ID` и `API_HASH` берутся в кабинете Telegram: https://my.telegram.org -> `API development tools`.
...
В этой же группе отправьте команду:
...
После этого бот сохранит связь между вашим Telegram user ID и `chat_id` группы. Эта связь нужна, чтобы кнопки управления знали, какую группу чистить.
